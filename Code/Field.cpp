...
Field::Field(Cell baseCell = CELL_WATER)
	: lives(0), shipsCount(0), gameProcess(false)
{
...
int Field::getLivesOfShipOn(int x, int y) const {
	...
	while (true) {
		...
		if (x + 1 < FL_MAX_SHIP_LEN && (f[x + 1][y] == CELL_WHOLE_SHIP || f[x + 1][y] == CELL_WOUNDED_SHIP))
			++x;
		else if (y + 1 < FL_MAX_SHIP_LEN && (f[x][y + 1] == CELL_WHOLE_SHIP || f[x][y + 1] == CELL_WOUNDED_SHIP))
			++y;
		else break;
	}
